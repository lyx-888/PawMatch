...
function SelectionToolbar({
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      {active ? (
        <>
          <p className="text-sm text-stone-700">Tap 2 or 3 pets to compare.</p>
          <button
            type="button"
            onClick={onCancel}
  ...
  return (
    <div className="sticky bottom-2 z-10 mt-2 flex items-center justify-between gap-3 rounded-full bg-stone-900 px-4 py-2 text-sm text-white shadow-lg">
      <span aria-live="polite">
        {count === 0
          ? `Pick ${COMPARE_MIN}–${COMPARE_MAX}`
          : `${count} selected${ready ? '' : ` · pick ${COMPARE_MIN - count} more`}`}
      </span>
    ...
    <li className="flex flex-col gap-2">
      {selectionMode ? (
        <button
          ...
          aria-label={`${selected ? 'Deselect' : 'Select'} ${pet.name} for compare`}
          className={cardClass}
        >
          {inner}
        </button>
