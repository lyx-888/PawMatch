...
export function CompareView({ pets }: Props): React.ReactElement {
  const rows = useMemo<Row[]>(
    () => [
      {
        label: 'Match',
        render: (_pet, match) => (match ? <TierPill tier={match.tier} score={match.score} /> : '—'),
      },
      {
        label: 'Species',
        render: (pet) => titleCase(pet.species) ?? '—',
      },
      {
        label: 'Breed',
      ...
      {
        label: 'Age',
        render: (pet) => formatAge(pet.ageMonths) ?? '—',
      },
      {
        label: 'Size',
        render: (pet) =>
      ...
      {
        label: 'Energy',
        render: (pet) => titleCase(pet.energyLevel) ?? '—',
      },
      {
        label: 'HDB-approved',
        render: (pet) => (pet.hdbApproved === null ? '—' : pet.hdbApproved ? 'Yes' : 'No'),
      },
      {
        label: 'Tags',
        render: (pet) =>
